...
def pep(session):
    ...
    sections_by_pep = main_section.find_all('td', attrs={'class': 'num'})
    results = [('Статус', 'Количество')]
    statuses = []
    for section in tqdm(sections_by_pep, colour='blue'):
        first_column_tag = section.find_previous_sibling('td')
        preview_status = first_column_tag.text[1:]
        pep_num_a_tag = find_tag(section, 'a')
        pep_num_link = urljoin(PEP_DOC_URL, pep_num_a_tag['href'])
